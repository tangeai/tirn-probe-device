                               ...
                               void *event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED)
    {
        ...
        {
            retry_count++;
            ESP_LOGW(TAG, "Wi-Fi 断开,重连中 %d/%d", retry_count, WIFI_MAX_RETRY);
            WX_VOIP_TRACEI(TAG, "Wi-Fi 断开后重连: retry=%d", retry_count);
            esp_wifi_connect();
        }
        else
        {
