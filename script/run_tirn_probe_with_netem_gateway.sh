...
Usage:
  ENDPOINT=... DEVICE_ID=... DEVICE_SECRET_KEY=... PEER_ID=... TOKEN=... \
    ...
    ./script/run_tirn_probe_with_netem_gateway.sh
  ...
  sh -eu -c '
    ...
    case "$COMMAND" in
      ...
      audio)
        ...
        /usr/local/bin/tirn-probe-device audio \
          --endpoint "$ENDPOINT" \
          --device-id "$DEVICE_ID" \
        ...
        ;;
      connect)
        printf "[netem-gateway] probe command: /usr/local/bin/tirn-probe-device connect --endpoint %s --device-id %s --device-secret-key *** --peer-id %s --token *** --iterations %s --connect-timeout-ms %s\n" \
        ...
        /usr/local/bin/tirn-probe-device connect \
          --endpoint "$ENDPOINT" \
          --device-id "$DEVICE_ID" \
          --device-secret-key "$DEVICE_SECRET_KEY" \
          --peer-id "$PEER_ID" \
        ...
        ;;
      timesync)
        printf "[netem-gateway] probe command: /usr/local/bin/tirn-probe-device timesync --endpoint %s --device-id %s --device-secret-key *** --peer-id %s --token *** --repeat %s --interval-ms %s --timeout-ms %s\n" \
          "$ENDPOINT" "$DEVICE_ID" "$PEER_ID" "$TIMESYNC_REPEAT" "$TIMESYNC_INTERVAL_MS" "$TIMESYNC_TIMEOUT_MS"
        retry_probe /usr/local/bin/tirn-probe-device timesync \
          --endpoint "$ENDPOINT" \
          --device-id "$DEVICE_ID" \
          --device-secret-key "$DEVICE_SECRET_KEY" \
          --peer-id "$PEER_ID" \
