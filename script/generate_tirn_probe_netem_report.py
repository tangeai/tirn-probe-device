...
def find_echo_audio(report_dir, stem, preferred_iteration=None):
    ...
    for directory, suffix in (("audio-echo-wav", "wav"), ("audio-echo", "opus")):
        for path in (report_dir / directory).glob(f"{stem}.iteration-*.{suffix}"):
            ...
            if found:
                # Opus is the raw output of this test run. A WAV can be a stale
                # publishing artifact left by an earlier report generation, so
                # never use it as the source of truth when the matching Opus is
                # available. The Feishu publisher converts the selected Opus to
                # a fresh WAV immediately before upload.
                format_priority = 1 if suffix == "opus" else 0
