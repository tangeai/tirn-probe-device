...
runner="$script_dir/run_accel_probe_netem_report.sh"
...
usage() {
  cat <<'USAGE'
Usage:
  ./script/run_accel_probe_connect_matrix.sh

The script reads test credentials from script/.env through
run_accel_probe_netem_report.sh. Environment variables accepted by that script
...
audio_processes() {
  {
    ...
    pgrep -af '[t]irtc_accel_device_probe[^[:space:]]*[[:space:]]+audio([[:space:]]|$)' 2>/dev/null || true
    pgrep -af '[r]un_accel_probe_.*audio.*\.sh' 2>/dev/null || true
  } | awk '!seen[$0]++'
}
...
audio_containers() {
  for container_id in $(docker ps -q 2>/dev/null); do
    ...
    if printf '%s\n' "$details" | grep -Eq \
      '(^|[[:space:]|])COMMAND=audio([[:space:]|]|$)|tirtc_accel_device_probe[^[:space:]"|]*[[:space:]"|]+audio([[:space:]"|]|$)|monitor_probe_image_audio_test'; then
