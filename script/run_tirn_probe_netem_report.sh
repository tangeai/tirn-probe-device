...
image=${PROBE_IMAGE:-tirn-probe-device-runner:audio-40ms}
...
Usage:
  ENDPOINT=... DEVICE_ID=... DEVICE_SECRET_KEY=... PEER_ID=... TOKEN=... \
    ./script/run_tirn_probe_netem_report.sh
...
run_case() {
  ...
  TIMESYNC_TIMEOUT_MS="$timesync_timeout_ms" \
    python3 "$script_dir/run_tirn_probe_case.py" --timeout-sec "$case_timeout_sec" --log "$log_file" -- \
      sh "$script_dir/run_tirn_probe_with_netem_gateway.sh"
  rc=$?
  set -e
...
python3 "\$script_dir/generate_tirn_probe_netem_report.py" \\
