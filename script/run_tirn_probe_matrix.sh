...
image=${PROBE_IMAGE:-tirn-probe-device-runner:20.04}
binary=${PROBE_BINARY:-./build/linux-x86_64/tirn-probe-device}
...
Usage:
  ENDPOINT=... DEVICE_ID=... DEVICE_SECRET_KEY=... PEER_ID=... TOKEN=... \
    ./script/run_tirn_probe_matrix.sh

Optional environment variables:
  PROBE_IMAGE=tirn-probe-device-runner:20.04
  CPU_LIMIT=0.5
  LOSSES=0,5,10,20
  DELAYS_MS=0,50,100
  ...
  for delay_ms in $(csv_to_words "$delays_ms"); do
    ...
    docker run --rm \
      ...
      sh -eu -c '
        ...
        case "$COMMAND" in
          audio)
            exec ./build/linux-x86_64/tirn-probe-device audio \
              --endpoint "$ENDPOINT" \
              --device-id "$DEVICE_ID" \
            ...
            ;;
          connect)
            exec ./build/linux-x86_64/tirn-probe-device connect \
              --endpoint "$ENDPOINT" \
              --device-id "$DEVICE_ID" \
              --device-secret-key "$DEVICE_SECRET_KEY" \
              --peer-id "$PEER_ID" \
            ...
            ;;
          timesync)
            exec ./build/linux-x86_64/tirn-probe-device timesync \
              --endpoint "$ENDPOINT" \
              --device-id "$DEVICE_ID" \
              --device-secret-key "$DEVICE_SECRET_KEY" \
              --peer-id "$PEER_ID" \
