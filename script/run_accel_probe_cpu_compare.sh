...
Usage:
  ENDPOINT=... DEVICE_ID=... DEVICE_SECRET_KEY=... PEER_ID=... TOKEN=... \
    ./script/run_accel_probe_cpu_compare.sh
...
for cpu in $(csv_to_words "$cpus"); do
  ...
  log_file="$report_dir/accel_probe_cpu_${tag}_audio_${audio_iterations}.log"
  ...
  TIMESYNC_TIMEOUT_MS="$timesync_timeout_ms" \
    sh "$script_dir/run_accel_probe_with_netem_gateway.sh" >"$log_file" 2>&1
