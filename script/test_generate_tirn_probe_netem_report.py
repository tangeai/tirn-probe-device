...
SCRIPT_PATH = Path(__file__).with_name("generate_tirn_probe_netem_report.py")
SPEC = importlib.util.spec_from_file_location("generate_tirn_probe_netem_report", SCRIPT_PATH)
REPORT = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(REPORT)


class ParseLogTest(unittest.TestCase):
    def test_find_echo_audio_prefers_raw_opus_over_stale_wav(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            report_dir = Path(temp_dir)
            opus_dir = report_dir / "audio-echo"
            wav_dir = report_dir / "audio-echo-wav"
            opus_dir.mkdir()
            wav_dir.mkdir()
            stem = "audio_delay_000_uplink_loss_10_downlink_loss_00"
            opus = opus_dir / f"{stem}.iteration-3.opus"
            stale_wav = wav_dir / f"{stem}.iteration-3.wav"
            opus.write_bytes(b"current-run-opus")
            stale_wav.write_bytes(b"stale-wav")

            selected = REPORT.find_echo_audio(report_dir, stem, 3)

        self.assertEqual(selected, opus)
